# -*- coding: utf-8 -*-
"""
Created on Mon Jun 14 10:35:59 2021
...
# The following varaibles contain values as described below:
# balance - the outstanding balance on the credit card
# annualInterestRate - annual interest rate as a decimal
# monthlyPaymentRate - minimum monthly payment rate as a decimal

# For each month, calculate statements on the monthly payment and remaining balance.
# At the end of the 12 months, print out the remaining balance. Be sure to print out no more
...
# Now write a program that calculates the minimum fixed monthly payment needed in order to pay off a credit card balance within 12 months.
# By a fixed monthly payment, we mean a single number which does not chnage each month, but instead is a constant amount that will be paid
# each month.
...
# for that month is made). The monthly payment must be a multiple of $10 and is the same for all months.
...
# You can try running your code locally so that the payment can be any dollar and cent amount (in other ways,
# the monthly payment is a multiple of $0.01). Does your code still work? It should, but you may notice that your
# code runs more slowly, espeically in cases with very large balances and interest rates.
# (Note: when your code is running on our servers, there are limits on the amount of computing time each submission
# is allowed, so your observations from running this experiment on the grading system might be limited to an error
...
# To recap the problem: we are searching for the smallest monthly payment such that we can pay off the entire
# balance within a year. What is a reasonable lower bound for this payment value? $10 is the obvious answer,
# but you can do better than that. If there was no interest, the debt can be paid off by monthly payments
# of one-twelfth of the original balance, so we must pay at least this much every month. One-twelfth of the
...
# because the interest was compounded on the balance we didn't pay off each month. So a good upper bound
# for the monthly payment would be one-twelfth of the balance, after having its interest compounded
